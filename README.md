...
App.py is the core Python file that handles the backend of this web application. It begins by configuring the Flask app, importing the neccesary functions, session handling, and SQLite database connection, ensuring each user’s data is stored securely and isolated through login‑protected routes. The file defines several key routes that power the app’s functionality:
...
- Delete route handles the logic of removing a video from table videos. It checks whether the video exists in the database. If it does, the function renders a confirmation page to ask the user to the confirm the deletion. If not, the function returns an apology error messgae.
